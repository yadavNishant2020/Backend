...
app.post("/submit", (req, res) => {
  ...
  const randomAdjectivNum = Math.floor(Math.random() * adj.length);
  const randomNounNum = Math.floor(Math.random() * noun.length);

  const randomAdjectiv = adj[randomAdjectivNum];
  const randomNoun = noun[randomNounNum];

  const BandName = randomAdjectiv + "-" + randomNoun;
  res.render("index.ejs", {
    band: BandName,
  });
});
